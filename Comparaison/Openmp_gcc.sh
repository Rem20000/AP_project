...
set output 'Comparaison entre code initial et parallélisé GCC.jpg'
...
set title 'Comparaison entre les perf du code initial et parallélisé (GCC)'

#tracer les histogrammes en utilisant les données de la deuxième colonne des fichiers.dat

plot 'gcc_-Ofast.dat' using 2:xtic(2) ti "-Ofast" lc rgb '#ADD8E6 ','gcc_-O0.dat' using 2:xtic(2) ti "-O0" lc rgb '#6A5ACD', 'gcc_-O1.dat' using 2:xtic(2) ti "-O1" lc rgb '#4169E1', 'gcc_-O2.dat' using 2:xtic(2) ti "-O2" lc rgb '#00008B', 'gcc_-O3.dat' using 2:xtic(2) ti "-O3" lc rgb '#708090', 'gcc_-Og.dat' using 2:xtic(2) ti "-Og" lc rgb '#00688B', 'OpenMp_gcc_-Ofast.dat' using 2:xtic(2) ti "-Ofast parallélisé" lc rgb 'orange','OpenMp_gcc_-O0.dat' using 2:xtic(2) ti "-O0 parallélisé " lc rgb '#CC5500', 'OpenMp_gcc_-O1.dat' using 2:xtic(2) ti "-O1 parallélisé " lc rgb '#FFB347', 'OpenMp_gcc_-O2.dat' using 2:xtic(2) ti "-O2 parallélisé " lc rgb '#FFFF66', 'OpenMp_gcc_-O3.dat' using 2:xtic(2) ti "-O3 parallélisé" lc rgb '#FF4500', 'OpenMp_gcc_-Og.dat' using 2:xtic(2) ti "-Og parallélisé" lc rgb '#FF7F50'
replot
